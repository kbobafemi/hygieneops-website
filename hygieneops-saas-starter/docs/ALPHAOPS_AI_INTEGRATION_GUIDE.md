...
Never place your OpenAI API key, Supabase service-role key, Meta token, X token or LinkedIn credentials inside frontend JavaScript. The frontend must call a backend function, and the backend function must call OpenAI or social media APIs.
...
8. Review conversation logs before enabling any automatic social media replies.
